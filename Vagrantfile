...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "dns" do |dns|
    ...
    dns.vm.provision "shell", inline: <<-SHELL
      ...
      cp -v /vagrant/dns/db.* /var/lib/bind
  ...
  config.vm.define "tierra" do |tierra|
    ...
    cp -v /vagrant/config/tierra/discovery.sistema.sol.ssl.conf /etc/apache2/sites-available

      # Copiar los recursos de nuestra web al Document Root
      # TODO
      cp -r /vagrant/config/tierra/discovery.sistema.sol /var/www/

      cp -v /vagrant/config/tierra/certificado_autofirmado.pem /etc/ssl/certs
      cp -v /vagrant/config/tierra/clave_privada.pem /etc/ssl/private
